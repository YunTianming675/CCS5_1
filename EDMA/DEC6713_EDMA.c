...
#pragma DATA_ALIGN(ping,128);        //变量ping存在cache（缓存）中，低7位都是0
#pragma DATA_ALIGN(pong,128);		//变量放到缓存里面的
#pragma DATA_ALIGN(outbuff,128);		//变量放到缓存里面的
int ping[BUFF_SZ];
int pong[BUFF_SZ];
int outbuff[BUFF_SZ];
...
#pragma DATA_ALIGN(ping_data,128);		//变量放到缓存里面的
#pragma DATA_ALIGN(pong_data,128);		//变量放到缓存里面的
...
extern far void vectors();		//远程调用汇编程序，汇编程序装载了中断向量表

void setupInterrupts(void);		//设置中断
void stopEdma(void); /* function used to stop EDMA */
void CACHE_clean1(Uint32 region,void *addr,Uint32 wordCnt);	//清除缓存
void CACHE_wbInvL2(void *blockPtr,Uint32 byteCnt,int wait);	//把byteCnt变量的值写回地址中去
/*-------------------------------------------------------------------------*/
...
//  CSL_init();
  ...
  CACHE_wbInvL2(&ping_data, 4, CACHE_WAIT);		//写回到缓存里面去
...
//  EDMA_clearPram(0x00000000);
   for(p1=(Uint32 *)0x01a00000,t=0;t<=0x8000>>2;t++) //EDA16个通道的参数放在这里，清零
   {
	   p1[t]=y6;
   }
...
//  cfgEdma = cfgEdmaPing;
  ...
  pram_base=(volatile Uint32 *)(LINKPRAMADDR+2*6*4); //选择链路参数
  t1=(Uint32)pram_base;
  pram_base[0]=x1;
  pram_base[1]=x2;
  pram_base[2]=x3;
  ...
  pram_base=(volatile Uint32 *)(LINKPRAMADDR+4*6*4);//通道4
  t2=(Uint32)pram_base;
  pram_base[0]=y1;
  pram_base[1]=y2;
  pram_base[2]=y3;
...
{
	register int t;
		/*disable interrupts,close EDMA channel before exit of program*/
		t=IER;
		t&=~(1<<8);
		IER=t;		//IRQ_enable(IRQ_EVT_EDMAINT //IRQ_disable(IRQ_EVT_EDMAINT

		t=0x00000000;
		edmar_base[CCER]=t;		//EDMA_REST(CCER,0X00000000);

		t=edmar_base[EER];
		t&=(1<<2);
		edmar_base[EER]=t;		//EDMA_disableChannel(hEdma)
		t=edmar_base[CIER];
		t&=~(1<<TCCINTNUM);
		edmar_base[CIER]=t;

		//EDMA_intDisable(TCCINTNUM);EDMA_intDisable(TCCINTNUM);
		t=edmar_base[CIPR];
		t&=~(1<<TCCINTNUM);
		edmar_base[CIPR]=t;		//EDMA_intClear(TCCINTNUM)

		//EDMA_close(hEdma)
		//EDMA_resetAll()

		t=0xffffffff;
		edmar_base[CIPR]=t;		//EDMA_REST(CIPR,0XFFFFFFFF)
		edmar_base[ECR]=t;		//EDMA_REST(ECR,0XFFFFFFFF)
}
